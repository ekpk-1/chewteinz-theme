window.modals = {
    ...
    handleSubscribeSubmit(e) {
        ...
        if (!listId) {
            ...
            errorText.textContent =
                "Newsletter signup is not configured. Please contact the store owner.";
            btn.textContent = "Get My 10% Off";
            btn.disabled = false;
            return false;
        }
    ...
    copyGiftCode() {
        var self = this;
        try {
            navigator.clipboard.writeText("CHEWFAN10").then(function () {
                self._showCopied();
            });
        } catch (e) {
    ...
    _showConfetti() {
        ...
        for (var i = 0; i < 50; i++) {
            ...
            html +=
                '<div class="absolute -top-4 w-2 h-2 opacity-0 animate-confetti-fall" style="left:' +
                left +
                "%;background-color:" +
                color +
                ";animation-delay:" +
    ...
    openNotify(flavorName) {
        ...
        document
            .getElementById("notify-form-section")
            .classList.remove("hidden");
        document
            .getElementById("notify-success-section")
            .classList.add("hidden");
        ...
        document.getElementById("notify-success-text").textContent =
            "We'll email you as soon as " + flavorName + " is available.";
    ...
    handleNotifySubmit(e) {
        ...
        var flavorName =
            document.getElementById("notify-flavor-name").textContent.trim() ||
            "this product";
        ...
        var settings = window.chewteinzSettings || {};
        var listId =
            settings.klaviyoNotifyListId || settings.klaviyoSubscribeListId;

        if (!listId) {
            errorEl.classList.remove("hidden");
            errorText.textContent =
                "Back-in-stock signup is not configured. Please contact the store owner.";
        ...
        var formBody =
            ...
            "&$source=" +
            encodeURIComponent("Back-in-stock notify: " + flavorName) +
            "&$fields=" +
            encodeURIComponent("$first_name,$source,notify_product") +
            "&notify_product=" +
            encodeURIComponent(flavorName);
    ...
    fetch(settings.makeWebhookUrl, {
        ...
        .then(function (res) {
            ...
            if (settings.supabaseUrl && settings.supabaseAnonKey) {
                return fetch(
                    settings.supabaseUrl + "/rest/v1/contact_submissions",
                    {
                        method: "POST",
                        headers: {
